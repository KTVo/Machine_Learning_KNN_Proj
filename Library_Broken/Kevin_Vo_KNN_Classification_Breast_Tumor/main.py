        ...
        for col in data_set:
            ...
            if random.randint(0, 1000) < 50:
                testDataSet.append({'id': int(col[0]), 'diagnosis': col[1], 'radius_mean': float(col[2]),
                                    'texture_mean': float(col[3]), 'smoothness_mean': float(col[6])});
                # if(col[1] == 'M'):
                #     testingSetM_radius_x.extend(float(col[2]))  # training set  M: -, Red
                #     testingSetM_smooth_y.extend(float(col[6]));  # training set  M: -, Red
                #     testingSetM_texture_z.extend(float(col[3]));  # training set  M: -, Red
